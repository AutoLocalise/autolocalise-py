...
def main():
    ...
    def track_api_calls(*args, **kwargs):
        ...
        api_call_count += 1
        print(f"  📡 API Call #{api_call_count}: {args[0] if args else 'Unknown endpoint'}")
    ...
    try:
        ...
        for original, translated in translations.items():
            cache_status = "FROM CACHE" if original in ["Hello, world!", "Submit"] else "FROM API"
    ...
    try:
        ...
        elapsed = (time.time() - start_time) * 1000
        print(f"  ✅ English to Spanish: 'Hello, world!' -> '{spanish}' ({elapsed:.1f}ms)")
        print(f"  📊 Cache size: {translator.cache_size()}")
    except Exception as e:
    ...
    try:
        ...
        elapsed = (time.time() - start_time) * 1000
        print(f"  ⚡ English to French: 'Hello, world!' -> '{french}' ({elapsed:.1f}ms) - FROM CACHE")
        print(f"  📊 Cache size: {translator.cache_size()}")
    except Exception as e:
    ...
    print(f"  Cache demonstrates significant performance improvement for repeated translations!")
